  ...
  return (
    <div className="flex flex-col md:flex-row flex-1 w-full mt-4 mb-1">
      ...
      <div className="w-full flex flex-1 flex-col pl-2">
        <h1 className="text-neutral-100 text-4xl font-semibold">{title}</h1>
        <h2 className="text-neutral-100 text-2xl mt-2">{subtitle}</h2>
        <p className="mt-auto text-neutral-300 text-lg">{date.toLocaleString()}</p>
        <a href={`/blog/${link.replace('.md', '')}`} className="text-green-300 hover:font-semibold">Read More</a>
      </div>
    </div>
  );
}
